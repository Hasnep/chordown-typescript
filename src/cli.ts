...
import { config_to_file_paths, get_commandline_arg, IConfig, read_config_file } from "./config";
...
const { input: input_file_paths, output: output_file_paths } = config_to_file_paths(chordown_config);
...
for (let i = 0; i < input_file_paths.length; i++) {
  ...
  if (output_formats.includes("tex")) {
    ...
    write_file_smart(export_tex(chordown_object, chordown_config), output_file_path);
